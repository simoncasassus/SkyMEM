...
def exec_prep_files(ZM):
    ...
    for afield in datapass:
        ...
        if ZM.GaussPB:
            ...
            rrs=np.sqrt((XX-hdr_sens['CRPIX1']+1)**2 + (YY-hdr_sens['CRPIX2']+1)**2)
    ...
    fits.writeto(ZM.workdir+"noise_mosaic.fits",noise_mosaic,hrstor,overwrite=True)
    fits.writeto(ZM.workdir+"attenuation.fits",PBmosaic,hrstor,overwrite=True)
...
def neglnlike(xfree,Restoring=False):
    ...
    for afield in ZM.datapass: 
        ...
        #kernel/=np.sum(kernel)
        dirtymodel=scipy_convolve(bim, kernel, mode='same') #,boundary='fill'
        residual=afield['data_image']-dirtymodel
...
def restore_skymem(ZM):
    ...
    fits.writeto(ZM.workdir+"residual_mosaic.fits",mos_residuals,ZM.rstor_head,overwrite=True)
    ...
    fits.writeto(ZM.workdir+"rstor_out.fits",restored,ZM.rstor_head,overwrite=True)

    restored_PBmult=restored*ZM.PBmosaic
    
    fits.writeto(ZM.workdir+"rstor_out_PBmult.fits",restored_PBmult,ZM.rstor_head,overwrite=True)
...
def run_scipy_optimize_minimize(ZM):
    ...
    result = op.minimize(neglnlike, xfree, method=method, jac=dneglnlike, tol=None,callback=None,  bounds=bounds, options=options)
...
def run_pygsl_minimize(ZM):
    ...
    sys = multiminimize.gsl_multimin_function_fdf(my_f, my_df, my_fdf, params, size)
...
def exec_ConjGrad(ZM):
    ...
    ZM.modout=modout
    fits.writeto(ZM.workdir+"mod_out.fits",modout,ZM.rstor_head,overwrite=True)
...
class Setup():
    def __init__(self,
        ...
        for a_attribute in initlocals.keys():
            if VerboseInit:
                print( "SkyMEM setting ",a_attribute," to ",initlocals[a_attribute])
